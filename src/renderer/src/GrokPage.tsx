...
function CpaCodexPanel({ snapshot, onSnapshot, notify }: Props): React.JSX.Element {
  ...
  return <div className="page-view accounts-view cpa-provider-view">
    ...
    <div className="toolbar">
      ...
      <div className="toolbar-group"><button onClick={() => void run(() => window.codexSwitcher.testCpaCodexAccounts(), 'CPA Codex 全部检测完成', false)} disabled={busy || snapshot.cpaCodexTesting.active}><TestTube2 size={16} />测试全部</button>{snapshot.cpaCodexTesting.active && <button className="danger-button" onClick={() => void window.codexSwitcher.cancelCpaCodexTests()}><Square size={15} />取消</button>}</div>
      {selected.size > 0 && <div className="toolbar-group toolbar-group-end selection-actions"><span>已选 {selected.size}</span><button onClick={() => void run(() => window.codexSwitcher.testCpaCodexAccounts(ids()), 'CPA Codex 选中检测完成', false)} disabled={busy || snapshot.cpaCodexTesting.active}><Play size={16} />测试选中</button><button onClick={() => setEnabled(true)} disabled={busy || snapshot.cpaCodexTesting.active}><Power size={16} />启用 .json</button><button onClick={() => setEnabled(false)} disabled={busy || snapshot.cpaCodexTesting.active}><PowerOff size={16} />停用 .json.0</button><button className="danger-button" onClick={remove} disabled={busy || snapshot.cpaCodexTesting.active}><Trash2 size={16} />删除选中</button></div>}
    </div>
...
function GrokPanel({ snapshot, onSnapshot, notify }: Props): React.JSX.Element {
  ...
  return <div className="page-view accounts-view grok-view cpa-provider-view">
    ...
    <div className="toolbar"><div className="toolbar-group"><button onClick={() => void run(() => window.codexSwitcher.scanGrokDirectory(), 'Grok 扫描完成')} disabled={busy || snapshot.grokTesting.active}><RefreshCw size={16} />重新扫描</button></div><div className="toolbar-group"><button onClick={() => void run(() => window.codexSwitcher.testGrokAccounts(), 'Grok 全部检测完成', false)} disabled={busy || snapshot.grokTesting.active}><TestTube2 size={16} />测试全部</button>{snapshot.grokTesting.active && <button className="danger-button" onClick={() => void window.codexSwitcher.cancelGrokTests()}><Square size={15} />取消</button>}</div>{selected.size > 0 && <div className="toolbar-group toolbar-group-end selection-actions"><span>已选 {selected.size}</span><button onClick={() => void run(() => window.codexSwitcher.testGrokAccounts(chosen()), 'Grok 选中检测完成', false)} disabled={busy || snapshot.grokTesting.active}><Play size={16} />测试选中</button><button onClick={() => setEnabled(true)} disabled={busy || snapshot.grokTesting.active}><Power size={16} />启用 .json</button><button onClick={() => setEnabled(false)} disabled={busy || snapshot.grokTesting.active}><PowerOff size={16} />停用 .json.0</button><button onClick={() => void exportAccounts('separate')} disabled={busy || snapshot.grokTesting.active}><Download size={16} />逐号导出</button><button onClick={() => void exportAccounts('bundle')} disabled={busy || snapshot.grokTesting.active}><FileArchive size={16} />合并导出</button><button className="danger-button" onClick={() => void remove()} disabled={busy || snapshot.grokTesting.active}><Trash2 size={16} />删除选中</button></div>}</div>
