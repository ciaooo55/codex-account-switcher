...
import { dateTime as time, resetCountdown, sourceFileName } from '../lib/format'
...
function CpaCodexPanel({ snapshot, onSnapshot, notify, requestConfirmation, onBusyChange, now = Date.now() }: Props): React.JSX.Element {
  ...
  async function run<T>(operation: () => Promise<T>, success: string | ((result: T) => void), reload = true): Promise<void> {
    ...
    try {
      ...
      if (reload) onSnapshot(await codexApi().getPageSnapshot('cpa'))
  ...
  async function runTest<T>(operation: () => Promise<T>, success: string | ((result: T) => void)): Promise<void> {
    try {
      const result = await operation()
      onSnapshot(await codexApi().getPageSnapshot('cpa'))
      if (typeof success === 'string') notify('ok', success)
      else success(result)
    } catch (error) {
      notify('error', error instanceof Error ? error.message : '操作失败')
  ...
  const setEnabled = (enabled: boolean, selectedIds = ids()): void => {
    void run(
      () => codexApi().setCpaCodexEnabled(selectedIds, enabled),
  ...
  const remove = async (removedIds = ids()): Promise<void> => {
    ...
    await run(async () => {
      const result = await codexApi().deleteCpaCodexAccounts(removedIds)
  ...
  const syncToLibrary = (selectedIds?: string[]): void => {
    void run(
      () => codexApi().syncCpaCodexToLibrary(selectedIds),
      (result) => notify(
  ...
  const handleCategoryAction = (
    ...
    if (action === 'select') {
      ...
      void runTest(
        () => codexApi().testCpaCodexAccounts(categoryIds, testMode),
  ...
  const handleGroupAction = (action: StatusCategoryAction, group: string): void => {
    const groupIds = snapshot.cpaCodexAccounts
      .filter((account) => {
        if (status && displayStatus(account.status) !== status) return false
    ...
    if (action === 'select') {
      ...
      void runTest(
        () => codexApi().testCpaCodexAccounts(groupIds, testMode),
        `CPA Codex 当前分组检测完成`
      )
    } else if (action === 'enable' || action === 'disable') {
      setEnabled(action === 'enable', groupIds)
    } else if (action === 'delete') {
      void remove(groupIds)
    }
  }
  return <PageView className="accounts-view cpa-provider-view">
    <section className="library-overview flex flex-wrap items-stretch gap-2 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface-0)] p-2">
    ...
    <StatusFilterStrip value={status} counts={facets.statusCounts} total={snapshot.cpaCodexAccounts.length} onChange={setStatus} label="CPA Codex 账号状态" onAction={handleCategoryAction} managedFiles disabled={busy || snapshot.cpaCodexTesting.active} groups={availableFacets.groups} groupValue={facetFilters.group} onGroupChange={(group) => setFacetFilters((current) => ({ ...current, group }))} onGroupAction={handleGroupAction} />
    <Toolbar>
      <ToolbarGroup><Button onClick={() => void run(() => codexApi().scanCpaCodexDirectory(), 'CPA Codex 扫描完成')} disabled={busy}><RefreshCw size={16} />重新扫描</Button><Button onClick={() => syncToLibrary()} disabled={busy || snapshot.cpaCodexTesting.active}><FolderSync size={16} />同步全部到 aa</Button></ToolbarGroup>
      <ToolbarGroup><CodexTestModeControl value={testMode} onChange={setTestMode} disabled={busy || snapshot.cpaCodexTesting.active} label="CPA Codex 检测模式" /><Button onClick={() => void runTest(() => codexApi().testCpaCodexAccounts(accounts.map((account) => account.id), testMode), `CPA Codex 当前筛选 ${accounts.length} 个账号${CODEX_TEST_MODE_SUCCESS[testMode]}`)} disabled={busy || snapshot.cpaCodexTesting.active || accounts.length === 0}><TestTube2 size={16} />测试当前页面全部</Button>{snapshot.cpaCodexTesting.active && <Button variant="danger" onClick={() => void codexApi().cancelCpaCodexTests()}><Square size={15} />取消</Button>}</ToolbarGroup>
    </Toolbar>
    {selected.size > 0 && <div className="selection-toolbar" aria-label="CPA Codex 选中账号操作">
      ...
      <Button onClick={() => void runTest(() => codexApi().testCpaCodexAccounts(ids(), testMode), `CPA Codex 选中账号${CODEX_TEST_MODE_SUCCESS[testMode]}`)} disabled={busy || snapshot.cpaCodexTesting.active}><Play size={16} />测试选中</Button>
    ...
    }} />)}{virtualAccounts.paddingBottom > 0 && <tr className="virtual-spacer" aria-hidden="true"><td colSpan={7} style={{ height: virtualAccounts.paddingBottom }} /></tr>}{!accounts.length && <EmptyTableRow colSpan={7}>没有匹配的 CPA Codex 账号</EmptyTableRow>}</tbody></table></TableWrap>
    {contextMenu && <div ref={contextMenuRef} className="account-context-menu" role="menu" aria-label="CPA Codex 账号管理" style={{ left: contextMenu.x, top: contextMenu.y }}>
      ...
      <button role="menuitem" onClick={() => { const id = contextMenu.account.id; setContextMenu(null); void runTest(() => codexApi().testCpaCodexAccounts([id], testMode), `CPA Codex ${CODEX_TEST_MODE_SUCCESS[testMode]}`) }}><TestTube2 size={15} />检测这个账号</button>
      ...
      <button role="menuitem" onClick={() => { const id = contextMenu.account.id; setContextMenu(null); void run(async () => { const result = await codexApi().revealManagedSource('cpa-codex', id); if (!result.ok) throw new Error(result.message) }, '已打开账号文件位置', false) }}><FolderOpen size={15} />打开文件位置</button>
...
function CpaCodexRow({ account, running, selected, toggle, now, testMode, virtualIndex, rowRef, openContextMenu }: { account: CpaCodexAccountSummary; running: boolean; selected: boolean; toggle: () => void; now: number; testMode: CodexTestMode; virtualIndex: number; rowRef?: (element: HTMLTableRowElement | null) => void; openContextMenu: (event: React.MouseEvent<HTMLTableRowElement>) => void }): React.JSX.Element {
  ...
  return <tr ref={rowRef} data-index={virtualIndex} className={`account-row status-row-${status}${running ? ' testing-row' : ''}${selected ? ' selected-row' : ''}${account.disabled ? ' disabled-file-row' : ''}`} tabIndex={0} onClick={toggle} onKeyDown={(event) => { if (event.target === event.currentTarget && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); toggle() } }} onContextMenu={openContextMenu}>
    ...
    <td><><AccountStatusBadge status={status} running={running} /><div className="status-detail" title={running ? CODEX_TEST_MODE_RUNNING[testMode] : account.detail}>{running ? CODEX_TEST_MODE_RUNNING[testMode] : account.detail}</div></></td>
...
function GrokPanel({ snapshot, onSnapshot, notify, requestConfirmation, onBusyChange, now = Date.now(), scope }: Props & { scope: 'library' | 'cpa' }): React.JSX.Element {
  ...
  async function run<T>(operation: () => Promise<T>, success: string | ((result: T) => void), reload = true): Promise<void> {
    ...
    try {
      const result = await operation()
      if (reload) onSnapshot(await codexApi().getPageSnapshot(cpa ? 'cpa' : 'grok'))
      if (typeof success === 'string') notify('ok', success)
      else success(result)
    }
  ...
  async function runTest<T>(operation: () => Promise<T>, success: string | ((result: T) => void)): Promise<void> {
    try {
      const result = await operation()
      onSnapshot(await codexApi().getPageSnapshot(cpa ? 'cpa' : 'grok'))
      if (typeof success === 'string') notify('ok', success)
      else success(result)
    } catch (error) {
      notify('error', error instanceof Error ? error.message : '操作失败')
  ...
  const remove = async (ids = chosen()): Promise<void> => {
    ...
    await run(async () => {
      const result = cpa
        ? await codexApi().deleteCpaGrokAccounts(ids)
        : await codexApi().deleteGrokAccounts(ids)
      if (!result.deleted) throw new Error('没有删除任何账号')
      setSelected(new Set())
    }, `Grok 账号已从${cpa ? ' CPA' : '本地账号库'}删除`)
  }
  const setEnabled = (enabled: boolean, ids = chosen()): void => { void run(() => cpa ? codexApi().setCpaGrokEnabled(ids, enabled) : codexApi().setGrokEnabled(ids, enabled), (result) => notify(result.changed ? 'ok' : 'warn', result.message)) }
  const testAccounts = (ids: string[]) => cpa ? codexApi().testCpaGrokAccounts(ids) : codexApi().testGrokAccounts(ids)
  const cancelTests = () => cpa ? codexApi().cancelCpaGrokTests() : codexApi().cancelGrokTests()
  const scan = () => cpa ? codexApi().scanCpaGrokDirectory() : codexApi().scanGrokDirectory()
  const exportAccounts = async (layout: 'separate' | 'bundle', ids = selected.size ? chosen() : accounts.map((item) => item.id)): Promise<void> => { if (ids.length) await run(async () => { const paths = await codexApi().exportGrokAccounts(ids, layout); if (!paths) throw new Error('已取消导出') }, layout === 'bundle' ? 'Sub2API 合并文件已导出' : 'CPA 单账号文件已导出', false) }
  const exportToCpa = async (ids = selected.size ? chosen() : accounts.map((item) => item.id)): Promise<void> => {
    if (!ids.length) return
    await run(
      () => codexApi().exportGrokAccountsToCpa(ids),
  ...
  const syncToLibrary = (ids?: string[]): void => {
    if (!cpa) return
    void run(
      () => codexApi().syncCpaGrokToLibrary(ids),
      (result) => notify(
        result.errors.length ? 'warn' : 'ok',
        `已同步到 aa：新增 ${result.imported} 个，重复跳过 ${result.skipped} 个${result.errors.length ? `，${result.errors.length} 个文件读取失败` : ''}`
      )
  ...
  const handleGroupAction = (action: StatusCategoryAction, group: string): void => {
    ...
    if (action === 'select') {
      ...
      void runTest(
        () => testAccounts(groupIds),
        `${cpa ? 'CPA ' : ''}Grok 当前分组检测完成`
      )
    } else if (cpa && (action === 'enable' || action === 'disable')) {
      setEnabled(action === 'enable', groupIds)
    } else if (action === 'delete') {
  ...
  return <PageView className="accounts-view grok-view cpa-provider-view">
    ...
    <Toolbar>
      <ToolbarGroup><Button onClick={() => void run(scan, `${cpa ? 'CPA ' : ''}Grok 扫描完成`)} disabled={busy || testing.active}><RefreshCw size={16} />重新扫描</Button>{cpa && <Button onClick={() => syncToLibrary()} disabled={busy || testing.active}><FolderSync size={16} />同步全部到 aa</Button>}</ToolbarGroup>
      <ToolbarGroup><Button onClick={() => void runTest(() => testAccounts(accounts.map((account) => account.id)), `Grok 当前筛选 ${accounts.length} 个账号检测完成`)} disabled={busy || testing.active || accounts.length === 0}><TestTube2 size={16} />测试当前页面全部</Button>{testing.active && <Button variant="danger" onClick={() => void cancelTests()}><Square size={15} />取消</Button>}</ToolbarGroup>
    </Toolbar>
    {selected.size > 0 && <div className="selection-toolbar" aria-label="Grok 选中账号操作">
      ...
      <Button onClick={() => void runTest(() => testAccounts(chosen()), 'Grok 选中检测完成')} disabled={busy || testing.active}><Play size={16} />测试选中</Button>
      ...
      <Button variant="danger" onClick={() => void remove()} disabled={busy || testing.active}><Trash2 size={16} />删除选中</Button>
    </div>}
    {/* progress shown globally in App header */}
    <div className="filter-row flex flex-wrap items-center gap-2"><SearchField icon={<Search size={16} />} value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索 Grok 邮箱、团队或等级" /><AccountFacetFilters label={`${cpa ? 'CPA ' : ''}Grok`} facets={availableFacets} value={facetFilters} onChange={setFacetFilters} /><Select aria-label={`${cpa ? 'CPA ' : ''}Grok 账号排序`} value={sort} onChange={(event) => setSort(event.target.value as AccountSortMode)}>{ACCOUNT_SORT_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select><span className="selection-count">显示 {accounts.length} / {sourceAccounts.length} · 已选 {selected.size}</span></div>
...
function GrokRow({
  ...
  return (
    ...
    >
      ...
      <td><><AccountStatusBadge status={status} running={running} /><div className="status-detail" title={running ? '检测完成后自动调整后缀' : account.detail}>{running ? '检测完成后自动调整后缀' : account.detail}</div></></td>
...
export function CpaPage(props: Props): React.JSX.Element {
  ...
  return <PageView className="cpa-view">
    <nav className="cpa-provider-tabs flex flex-wrap items-center gap-1.5 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface-1)] p-1.5" aria-label="CPA 账号类型">
