...
function CpaCodexPanel({ snapshot, onSnapshot, notify, requestConfirmation, onBusyChange, now = Date.now() }: Props): React.JSX.Element {
  ...
  return <div className="page-view accounts-view cpa-provider-view">
    ...
    <div className="toolbar">
      <div className="toolbar-group"><button onClick={() => void run(() => codexApi().scanCpaCodexDirectory(), 'CPA Codex 扫描完成')} disabled={busy}><RefreshCw size={16} />重新扫描</button><button onClick={() => syncToLibrary()} disabled={busy || snapshot.cpaCodexTesting.active}><FolderSync size={16} />同步全部到 aa</button></div>
      <div className="toolbar-group"><CodexTestModeControl value={testMode} onChange={setTestMode} disabled={busy || snapshot.cpaCodexTesting.active} label="CPA Codex 检测模式" /><button onClick={() => void runTest(() => codexApi().testCpaCodexAccounts(accounts.map((account) => account.id), testMode), `CPA Codex 当前筛选 ${accounts.length} 个账号${CODEX_TEST_MODE_SUCCESS[testMode]}`)} disabled={busy || snapshot.cpaCodexTesting.active || accounts.length === 0}><TestTube2 size={16} />测试当前页面全部</button>{snapshot.cpaCodexTesting.active && <button className="danger-button" onClick={() => void codexApi().cancelCpaCodexTests()}><Square size={15} />取消</button>}</div>
    </div>
