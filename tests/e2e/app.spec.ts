...
test.describe('Codex Account Switcher Electron workflow', () => {
  ...
  test('scans, tests, switches, repairs sessions and restores API mode', async () => {
    ...
    const importPreview = page.getByRole('dialog', { name: '导入预检' })
    await expect(importPreview).toBeVisible()
    await expect(importPreview).toContainText('新增 2')
    await importPreview.getByRole('button', { name: '一键检测全部' }).click()
    await expect(page.getByText('导入凭证检测完成：2 个账号')).toBeVisible()
    await expect(importPreview.locator('.preview-test-status.status-valid')).toHaveCount(2)
    await importPreview.getByLabel('凭证检测结果').selectOption('valid')
    await expect(importPreview.getByRole('option', { name: '有效 2' })).toHaveCount(1)
    await expect(importPreview.getByRole('row')).toHaveCount(3)
    await page.screenshot({ path: join(process.cwd(), 'test-results', 'import-preview-tested.png'), fullPage: true })
    await importPreview.getByRole('button', { name: '确认写入 aa' }).click()
    await expect(page.getByText('导入完成：新增 2，更新 0，跳过 0')).toBeVisible()
    await expect(page.getByRole('row', { name: /folder-e2e@example\.com/ })).toBeVisible()
    expect(await readdir(join(userData, 'aa', 'codex'))).toContain('folder-e2e@example.com_plus.json')
    expect(await readdir(join(userData, 'aa', 'grok'))).toContain('grok-e2e@example.com_supergrok.json')
    ...
    expect(await readdir(join(userData, 'aa', 'codex'))).not.toContain('folder-e2e@example.com_plus.json')
    ...
    const beforeCodexTestAll = requests.length
    ...
    await page.keyboard.press('Escape')
    expect(requests.slice(beforeCodexTestAll).sort()).toEqual(['/compact', '/compact', '/usage', '/usage'])
    ...
    const categoryMenu = page.getByRole('menu', { name: 'Codex 账号状态分类操作' })
    await expect(categoryMenu.getByRole('menuitem', { name: '测试该分类' })).toBeVisible()
    await expect(categoryMenu.getByRole('menuitem', { name: '删除该分类全部账号' })).toBeVisible()
    await page.waitForTimeout(180)
    await expect(categoryMenu).toBeVisible()
    await page.screenshot({ path: join(process.cwd(), 'test-results', 'status-category-menu.png') })
    await page.keyboard.press('Escape')
    const groupFilters = page.getByRole('group', { name: 'Codex 账号状态分组筛选' })
    await groupFilters.getByRole('button', { name: /未分组/ }).click({ button: 'right' })
    const groupMenu = page.getByRole('menu', { name: 'Codex 账号状态分组操作' })
    await expect(groupMenu.getByRole('menuitem', { name: '选择该分组' })).toBeVisible()
    await expect(groupMenu.getByRole('menuitem', { name: '测试该分组' })).toBeVisible()
    await expect(groupMenu.getByRole('menuitem', { name: '删除该分组全部账号' })).toBeVisible()
    await page.waitForTimeout(180)
    await page.screenshot({ path: join(process.cwd(), 'test-results', 'group-context-menu.png') })
    await page.keyboard.press('Escape')
    ...
    const pastedPreview = page.getByRole('dialog', { name: '导入预检' })
    await expect(pastedPreview).toBeVisible()
    await expect(pastedPreview).toContainText('pasted-e2e@example.com')
    await pastedPreview.getByRole('button', { name: '确认写入 aa' }).click()
    ...
    await page.screenshot({ path: join(process.cwd(), 'test-results', 'accounts-ui-compact-dark.png'), fullPage: true })
    ...
    await page.getByRole('button', { name: '查看请求' }).click()
    const activityDialog = page.getByRole('dialog', { name: 'API 服务活动' })
    await expect(activityDialog).toBeVisible()
    await page.screenshot({ path: join(process.cwd(), 'test-results', 'api-service-activity.png'), fullPage: true })
    await activityDialog.getByRole('button', { name: '关闭 API 服务活动' }).click()
    ...
    await page.getByRole('button', { name: '编辑模型 e2e-upstream-model' }).first().click()
    const routeDialog = page.getByRole('dialog', { name: '模型路由' })
