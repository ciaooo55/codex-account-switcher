...
- 支持单击整行选择账号、`Ctrl` 追加多选、复选框全选和多选，并提供明确的整行选中高亮；删除前二次确认，删除会同步删除或重写 `aa` 中的托管文件，一文件多账号时只移除选中的账号，外部原始文件不受影响。
...
- 打包版可直接检查 GitHub Release；安装包下载到当前 Windows 用户实际的“下载”文件夹，经 SHA-512 校验后覆盖安装，并在安装结束后自动删除。
- 安装或升级时会自动退出托盘中的旧版本；若旧进程未正常退出，安装器会结束其进程树后继续覆盖安装。
...
- `Codex-Account-Switcher-Setup-0.7.0.exe`：安装版
- `Codex-Account-Switcher-Portable-0.7.0.exe`：便携版
