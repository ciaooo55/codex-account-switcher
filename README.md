...
- 安装或升级时会先通知托盘中的旧版本退出；应用内更新会按已安装 EXE 的完整路径等待所有旧进程，超时后只结束该路径对应的进程再继续覆盖安装，不会把安装器自身当成应用进程。
...
- `Codex-Account-Switcher-Setup-0.7.1.exe`：安装版
- `Codex-Account-Switcher-Portable-0.7.1.exe`：便携版
