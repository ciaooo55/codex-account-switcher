...
- Codex 本地库和 CPA Codex 均提供三种检测模式：“仅额度”只读取 usage 与重置时间，不发送模型请求；“完整测试”查询额度后再调用 compact 验证真实可用性；“仅刷新”只旋转 OAuth token，并保留上次额度快照。三种模式都支持当前筛选结果、选中账号、右键单账号检测、并发进度和取消。
- 并发检测中的账号使用稳定状态指示，每完成一个账号立即增量显示状态、额度、重置时间和刷新时间；状态文件在主进程内缓存，批量任务不会为每个账号重复读取整份结果文件。
- 检测状态和额度结果持久保存到下次检测；状态标签只显示当前实际存在的分类，并可按当前状态组动态筛选实际存在的账号类型、邮箱域名与失效/错误原因。搜索、筛选和排序只作用于当前页面，“测试当前页面全部”只提交筛选后仍可见的账号。
- 账号可维护本机别名、分组、多个标签和备注；这些信息独立保存，不写入凭证 JSON。列表、搜索、动态筛选和定时切换候选都支持别名/分组/标签，重复账号跨 aa/CPA 目录仍共用同一份元数据。
- Codex 本地库、Grok 本地库、CPA Codex 与 CPA Grok 的凭据和删除记录彼此独立；只有用户主动执行“导出到 CPA”才会把选定的本地账号写入 CPA 目录，也可手动将 CPA 新账号只增量同步到 `aa`。同一提供商下匹配到的相同凭证会在 aa 与 CPA 状态库之间同步最新检测状态、额度和重置时间，但不会因此复制或删除凭证文件。
- 当前 `auth.json` 匹配到的 Codex 账号在所有排序方式下都会置顶，并使用独立的青色整行高亮、“正在使用”徽标和摘要标记；顶部摘要显示账号、等级、5 小时/周额度与恢复倒计时，并可只刷新当前账号额度。
...
- 支持单选、多选或批量把规范托管文件从 `.json` 改为 `.json.0`，使 CPA 暂停读取；再次启用会恢复 `.json`。只有从 CPA 页面发起的检测会管理后缀：5 小时或周额度耗尽自动改为 `.json.无用量`，明确失效或无权限改为 `.json.无权限`，后续检测恢复有效会自动还原 `.json`。从 Codex/Grok 本地账号库发起的检测只同步状态与额度，绝不改动 CPA 文件名或启停状态。
...
- CPA Codex/Grok 共享凭证库：自动读取 CLIProxyAPI `config.yaml` 的 `auth-dir`，并依次检查环境变量、已保存目录、当前用户 `.cli-proxy-api` 和 `E:\home\<当前用户名>\.cli-proxy-api`
...
对话管理直接读取 Codex rollout 的 `source`、`parent_thread_id` 以及状态库中的 `thread_spawn_edges`，不会按标题猜测子代理。保守清理只会将已标记为 `closed`、超过 60 分钟没有写入、没有任何派生子任务的 `thread_spawn` 会话移入 Windows 回收站；主对话、仍为 `open` 的可恢复代理以及关系不明确的会话均会跳过。
